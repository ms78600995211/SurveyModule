...
import com.apogee.gnss.pdaParams.PDAParamsModel;
 ...
 * @author admin
 */
@WebServlet(name = "PDAParamsController", urlPatterns = {"/PDAParamsController"})
public class PDAParamsController extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        PDAParamsModel model = new PDAParamsModel();
        ...
        try {
            ...
            if (submit.equals("Submit")) {
                ...
                model.savePDAParams(IP, portNo, url, username, passwd, defaultConfig, mountPoint, name, remark);
            }
            request.setAttribute("message", model.getMessage());
            request.setAttribute("msgbgcolor", model.getColor());
            request.setAttribute("getPDAParams", model.getPDAParams(portNum));
            request.setAttribute("allPortNumber", model.allPortNumber());
            model.closeConnection();
            request.getRequestDispatcher("pdaparams").forward(request, response);
        } catch (Exception e) {
            System.out.println("com.apogee.gnss.controller.PDAParamsController.doGet()" + e);
        }
